...
test.describe('Chat', () => {
  ...
  test('connects and renders main layout', async () => {
    ...
    await expect(sharedPage.getByRole('navigation', { name: 'Channels' })).toBeVisible();
    await expect(sharedPage.locator('#message-input')).toBeVisible();
    await expect(sharedPage.getByRole('complementary', { name: 'Users' })).toBeVisible();

    // Verify both channels appear in sidebar
    await expect(sharedPage.getByRole('button', { name: '#general' })).toBeVisible();
    await expect(sharedPage.getByRole('button', { name: '#random' })).toBeVisible();
  });

  test('shows different topics when switching channels', async () => {
    // Click #general - verify its topic
    await sharedPage.getByRole('button', { name: '#general' }).click();
    await expect(sharedPage.getByLabel('topic-input')).toHaveValue('General discussion channel');

    // Switch to #random - verify different topic
    await sharedPage.getByRole('button', { name: '#random' }).click();
    await expect(sharedPage.getByLabel('topic-input')).toHaveValue('Random off-topic chat');
