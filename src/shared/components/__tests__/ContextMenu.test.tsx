...
describe('ContextMenu', () => {
  ...
  describe('Private message user list population', () => {
    ...
    it('should create and switch to the priv channel when clicking Priv without touching the users store', () => {
      vi.spyOn(ContextMenuContext, 'useContextMenu').mockReturnValue(
        createContextMenuMock({ contextMenuItem: 'otherUser' })
      );
      vi.spyOn(settings, 'getCurrentNick').mockReturnValue('currentUser');
      ...
      const mockSetCurrentChannelName = vi.spyOn(settings, 'setCurrentChannelName').mockImplementation(() => {});
      ...
      // Should create and switch to the priv channel; participants are derived from the window name, not stored
      expect(mockSetAddChannel).toHaveBeenCalledWith('otherUser', ChannelCategory.priv);
      expect(mockSetCurrentChannelName).toHaveBeenCalledWith('otherUser', ChannelCategory.priv);
